...
Class Archive_Breadcrumb_View extends ArchiveViewAbstract {
	...
	public function render_module_back( $attr, $column_class ) {
		...
		return
			"<div {$this->element_id($attr)} class='jeg_archive_breadcrumb_wrapper {$attr['scheme']} {$attr['el_class']}'>
				{$style}
	...
	public function render_module_front( $attr, $column_class ) {
		...
		return
			"<div {$this->element_id($attr)} class='jeg_archive_breadcrumb_wrapper {$attr['scheme']} {$attr['el_class']}'>
				{$style}
                <div class=\"jeg_breadcrumbs jeg_breadcrumb_container\">{$breadcrumb}</div>
            </div>";
	}
	...
	public function build_breadcrumb( $id ) {
		...
		$breadcrumb   = array();
		$breadcrumb[] = $this->breadcrumb_text( esc_url( home_url( '/' ) ), esc_html__( 'Home', 'epic-ne' ) );

		if ( is_author() ) {
			$breadcrumb[] = $this->breadcrumb_text( '', esc_html__( 'Author', 'epic-ne' ) );
			$breadcrumb[] = $this->breadcrumb_text( '', get_the_author_meta( 'display_name', $id ), $this->last_link_class );
		} else {
			if ( is_category() ) {
				$breadcrumb[] = $this->breadcrumb_text( '', esc_html__( 'Category', 'epic-ne' ) );
			} elseif ( is_tag() ) {
				$breadcrumb[] = $this->breadcrumb_text( '', esc_html__( 'Tag', 'epic-ne' ) );
			}
		...
		return apply_filters( 'epic_native_breadcrumb_category', $breadcrumb, $id );
	}
