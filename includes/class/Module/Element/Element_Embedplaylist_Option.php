...
Class Element_Embedplaylist_Option extends ModuleOptionAbstract
{
    ...
    {
        $api = epic_get_option( 'youtube-api', '' );

        if ( ! $api ) {
            $this->options[] = array(
                'type'        => 'alert',
                'param_name'  => 'compatible_column_notice',
                'heading'     => esc_html__( 'YouTube API ', 'epic-ne' ),
                'description' => esc_html__( 'You are not yet enter you YouTube API, please go to Customizer > Epic : Global API Option > Google API Key > YouTube API Key', 'epic-ne' ),
                'std'         => 'error'
            );
        }
